<template>
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
        ...
        <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			...
			<view v-if="queryIndex==0" class="search-form round">
				...
				<input v-model="searchForm.canpinmingcheng" type="text" placeholder="餐品名称" ></input>
			</view>
	...
	<scroll-view scroll-x="true" class="tabView" :style='{"padding":"8rpx 0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"rgba(0,0,0,1)","backgroundColor":"#e2e2e2","borderRadius":"0","borderWidth":"0","width":"100%","borderStyle":"solid","height":"auto"}'>
        	<view :style='categoryName===item.canpinfenlei?{"padding":"0 28rpx","boxShadow":"0 0 12rpx rgba(0,0,0,.3)","margin":"0 12rpx","borderColor":"rgba(0,0,0,1)","backgroundColor":"#333","color":"#fff","borderRadius":"8rpx","borderWidth":"0","width":"auto","lineHeight":"68rpx","fontSize":"28rpx","borderStyle":"solid"}:{"padding":"0 20rpx","boxShadow":"0 0 12rpx rgba(0,0,0,.3)","margin":"0 12rpx","borderColor":"rgba(0,0,0,1)","backgroundColor":"#fff","color":"#333","borderRadius":"8rpx","borderWidth":"0","width":"auto","lineHeight":"68rpx","fontSize":"28rpx","borderStyle":"solid"}' class="tab" v-for="(item,index) in categoryList" :key="index" :class="categoryName===item.canpinfenlei?'tabActive':''" @tap="categoryClick(item.canpinfenlei)">{{item.canpinfenlei}}</view>
        </scroll-view>
		...
		<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"#efefef"}'>
			<view @tap="onDetailTap(product)" class="uni-product" :style='{"borderRadius":"8rpx","backgroundColor":"#fff"}' v-for="(product,index) in list" :key="index">
								                				                                <view class="uni-product-title" :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"#333","textAlign":"left"}'>{{product.canpinmingcheng}}</view>
					...
					<text v-if="isAuth('tejiacanpin','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
					<text v-if="isAuth('tejiacanpin','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
				</view>
			</view>
		</view>
		
		
		<button v-if="isAuth('tejiacanpin','新增')" class="add-btn" @click="onAddTap()">新增</button>
...
<script>
	export default {
		...
		async onShow() {
			let res = await this.$api.list('canpinfenlei', {page:1,limit:100});
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.categoryName!='全部'){
					params.canpinfenlei = '%' + this.categoryName + '%'
				}
				let res = await this.$api.list(`tejiacanpin`, params);
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('tejiacanpin', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				let searchForm = {
					...
					searchForm['canpinfenlei'] = '%' + this.searchForm.canpinfenlei + '%'
																																																												...
																																																												let res = await this.$api.list(`tejiacanpin`, searchForm);
				// 如果是第一页数据置空
